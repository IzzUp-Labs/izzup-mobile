    ...
    Api.getExtraRequests().then((value) {
      ...
      setState(() {
        ...
        if(context.mounted) {
          ...
          requestProvider.addJobRequestList(requests);
        }
      });
    });
  }
    ...
    return Scaffold(
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: Consumer<JobRequestWithVerificationCodeNotifier>(
            builder: (context, jobList, child) {
              return ListView.builder(
                itemCount: jobList.requests.length + 1 + (jobList.requests.isEmpty ? 1 : 0),
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      jobList.requests[index - 1].company.name,
                                      style: const TextStyle(
                                        color: Colors.white,
                                        fontSize: 20,
                                        fontWeight: FontWeight.bold,
                              ...
                              Icon(
                                getIconFromStatus(jobList.requests[index - 1].status),
                                color: Colors.white,
                              ),
                            ],
                          )
